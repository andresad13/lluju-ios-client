...
- (void)setCalendar:(CGRect)frame withCalendarHeight:(CGFloat)calendarHeight withShowSingle:(BOOL)isShowSingle
{
    [self initCalendarHeaderViewWithFrame:frame withCalendarHeaderViewHeight:kCalendarHeaderViewHeight];
          [self initFSCalendarScrollViewWithFrame:frame withCalendarScrollViewHeight:calendarHeight-kCalendarHeaderViewHeight withShowSingle:isShowSingle];
...
- (void)initCalendarHeaderViewWithFrame:(CGRect)frame withCalendarHeaderViewHeight:(CGFloat)calendarHeaderViewHeight {
    ...
    NSMutableArray *days = [[dateFormatter shortStandaloneWeekdaySymbols] mutableCopy];// 非常短，可以改其它枚举值
...
- (void)handleSwipeFrom:(UISwipeGestureRecognizer *)recognizer {
    ...
    if (recognizer.direction==UISwipeGestureRecognizerDirectionUp) {
      ...
      [self showSingleWeekView:YES];
        if (_fsDelegate && [_fsDelegate respondsToSelector:@selector(swipeUp)]) {
            [_fsDelegate swipeUp];
    ...
    if (recognizer.direction==UISwipeGestureRecognizerDirectionDown) {
        ...
        if (_fsDelegate && [_fsDelegate respondsToSelector:@selector(swipedown)]) {
           [_fsDelegate swipedown];
        }
...
- (void)refreshToCurrentDate:(BOOL)bSele {
    ...
    [self.calendarScrollView refreshToCurrentDate:bSele];
}
  
- (void)moveScrolAnima:(BOOL)bLeft
{
    [self.calendarScrollView moveScrolAnima:bLeft];
}
/*
