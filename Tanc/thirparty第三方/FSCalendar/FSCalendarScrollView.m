...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if ([super initWithFrame:frame]) {
        ...
        curSeleYear = [[NSDate date] dateYear];
        curSeleMon = [[NSDate date] dateMonth];
...
- (void)layoutCollectionViewsDataWithCell:(FSCalendarCell *)cell IndexPath:(NSIndexPath *)indexPath withDataIndex:(NSInteger)index {
    ...
    if (indexPath.row < firstWeekday) {// 上月末尾的几天
        ...
        if (index == 1) {
            ...
            if ((monthInfo.month == curSeleMon) && (monthInfo.year == curSeleYear) && (self.currentDateNumber+firstWeekday-1 == indexPath.row)) {
                //选中的
                cell.solarDateLabel.textColor = [UIColor whiteColor];
                  cell.currentSelectView.backgroundColor = [UIColor colorWithRed:99/255.0 green:177/255.0 blue:94/255.0 alpha:1.0];
            }
            else if((monthInfo.month == [[NSDate date] dateMonth]) && (monthInfo.year == [[NSDate date] dateYear]) && (indexPath.row == [[NSDate date] dateDay] + firstWeekday - 1))
            {//当天
                cell.currentSelectView.backgroundColor =  [UIColor colorWithRed:246/255.0 green:248/255.0 blue:252/255.0 alpha:1.0];
                cell.solarDateLabel.textColor = [UIColor colorWithRed:99/255.0 green:177/255.0 blue:94/255.0 alpha:1.0];
            }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    curSeleYear = monthInfo.year;
    curSeleMon = monthInfo.month;
    // 记录当前选中的日期number
...
// 回到今天
- (void)refreshToCurrentDate:(BOOL)bSele {
    ...
    self.currentMonthDate = [NSDate date];
    self.currentDateNumber = bSele ? [self.currentMonthDate dateDay] : -1;
...
- (void)moveScrolAnima:(BOOL)bLeft
{
    [UIView animateWithDuration:0.3 animations:^{
        CGPoint point = self.contentOffset;
        if (bLeft) {
            // self.scrol
            point.x =  point.x - WIDTH < 0? WIDTH * 3 :  point.x - WIDTH;
        ...
        {
            point.x =  point.x + WIDTH >  3 * WIDTH ?  0 : point.x + WIDTH;
            self.contentOffset = point;
        ...
        //
        // [self setScrollViewContentOffset];
        if (self.contentOffset.x == kScreenWidth && self.contentOffset.y == 0.00f && self.isShowSingle) {
